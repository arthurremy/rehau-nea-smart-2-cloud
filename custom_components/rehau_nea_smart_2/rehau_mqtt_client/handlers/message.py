...
# Diagnostic: log the first time we see each (topic, message-type) so the MQTT
# traffic is visible in the HA UI log without spamming it every message.
_SEEN = set()


def _diag(topic, mtype):
    key = (topic, mtype)
    if key not in _SEEN:
        _SEEN.add(key)
        _LOGGER.warning("REHAU MQTT message received: topic=%s type=%s", topic, mtype)
...
async def handle_referential(message, client):
    ...
    _LOGGER.warning("REHAU MQTT referentials received and stored (writes enabled)")
